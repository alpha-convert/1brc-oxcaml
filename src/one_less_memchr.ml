...
let compute_record (buf @ shared) tbl ~start_idx ~semic_idx =
  let #(~town_hash,~temp,~newline_idx) = parse_line buf ~start_idx ~semic_idx in
  ...
  Parallel_scheduler.parallel scheduler ~f:(fun parallel ->
    ...
    let next _ () st  =
      ...
      if start_idx >= end_idx then
        ...
        let newline_idx = compute_record (magic_shared buf) tbl ~start_idx ~semic_idx in
